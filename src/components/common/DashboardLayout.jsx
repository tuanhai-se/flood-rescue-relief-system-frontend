...
export default function DashboardLayout() {
  ...
  const currentPage =
    filteredNav.find((n) => location.pathname === n.path) ||
    filteredNav.find(
      (n) => location.pathname.startsWith(n.path) && n.path !== "/dashboard",
    );
  ...
  return (
    <div className="h-screen flex overflow-hidden bg-gray-50">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-[72px]"} bg-gradient-to-b from-[#0c1e3a] to-[#0a1628] text-white flex flex-col transition-all duration-300 shrink-0`}
      >
        {/* Logo */}
        ...
        <div className="px-3 pb-2">
          <Link
            to="/"
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] text-gray-500 hover:bg-white/5 hover:text-gray-300 transition"
          >
                  ...
                  <div className="overflow-y-auto flex-1">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((n) => (
                        <div
                          key={n.id}
                          className={`px-4 py-3 border-b last:border-0 hover:bg-gray-50 transition cursor-pointer ${!n.is_read ? "bg-blue-50/50" : ""}`}
                        >
