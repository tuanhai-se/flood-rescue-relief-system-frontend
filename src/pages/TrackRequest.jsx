...
import { Search, MapPin, Clock, Users, Phone, CheckCircle, AlertTriangle,
         Truck, Waves, ArrowLeft, Navigation, FileText, Edit2, X, Save, UserX } from 'lucide-react';
        ...
        {request && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              {/* Rescue team đã hoàn thành, chờ coordinator đóng đơn */}
              {request.rescue_team_confirmed && request.status !== 'completed' && (
                <div className="mt-4 pt-4 border-t">
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
                    <CheckCircle className="text-emerald-600 shrink-0 mt-0.5" size={20} />
                    <div>
                      <p className="font-semibold text-emerald-800 text-sm">Đội cứu hộ đã hoàn thành nhiệm vụ</p>
                      <p className="text-xs text-emerald-600 mt-0.5">Đang chờ coordinator xác nhận và đóng đơn. Vui lòng chờ thêm.</p>
                    </div>
                  </div>
                </div>
              )}

              {/* Citizen confirm rescue — chỉ hiện sau khi rescue team đã xác nhận hoàn thành */}
              {(request.rescue_team_confirmed || request.status === 'completed') && (
                <div className="mt-4 pt-4 border-t space-y-3">
                  {request.citizen_confirmed ? (
                    ...
                    <ConfirmRescueButton trackingCode={request.tracking_code} onConfirmed={() => setRequest(prev => ({ ...prev, citizen_confirmed: true, citizen_confirmed_at: new Date().toISOString() }))} />
                  )}
                  {/* Rescued by other citizen */}
                  {!request.citizen_confirmed && request.status !== 'completed' && (
              ...
              {/* Rescued by other: also show when pending/verified/assigned */}
              {['pending', 'verified', 'assigned'].includes(request.status) && (
                <div className="mt-4 pt-4 border-t">
                  <RescuedByOtherButton
                    trackingCode={request.tracking_code}
                    onCancelled={() => setRequest(prev => ({ ...prev, status: 'cancelled' }))}
