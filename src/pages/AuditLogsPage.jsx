...
export default function AuditLogsPage() {
  ...
  useEffect(() => { load(); }, [filters.page, filters.action]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-800 flex items-center gap-2">
          <ClipboardList size={22} className="text-gray-500" /> Nhật ký hoạt động
        </h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400">Chưa có nhật ký</td></tr>
              ) : (
                logs.map(log => (
                  <tr key={log.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{formatDate(log.created_at)}</td>
                    <td className="px-4 py-3 font-medium text-gray-800">{log.full_name || log.user_id || '—'}</td>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded-full">{log.action}</span>
                    </td>
