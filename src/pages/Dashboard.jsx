...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        // Auto-load weather theo vị trí GPS hiện tại (ưu tiên) hoặc province_id
        if (configured) {
            ...
            navigator.geolocation.getCurrentPosition(
              async (pos) => {
                try {
                  ...
                  // Tìm province gần nhất theo coords để load forecast
                ...
                } catch { /* fallback to province */ }
              },
              () => {
                // Geolocation bị từ chối → fallback theo province_id
                if (user?.province_id &&
                    (user?.role === 'coordinator' || (user?.role === 'rescue_team' && user?.is_team_leader) || user?.role === 'manager')) {
  ...
  // Load weather khi chọn tỉnh (forceLoad=true để bypass state chưa update)
  ...
  return (
    <div className="space-y-6">
      ...
      {user?.role === 'manager' && (
        <div className="space-y-6">
          <div>
            <h2 className="text-base font-semibold text-gray-700 mb-3">Tổng quan tài nguyên</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <StatCard icon={Truck} label="Tổng phương tiện" value={resourceOverview?.vehicles?.total ?? '—'} color="blue" />
              <StatCard icon={Truck} label="Đang sử dụng" value={resourceOverview?.vehicles?.in_use ?? '—'} color="orange" />
              <StatCard icon={Archive} label="Kho hàng" value={resourceOverview?.warehouses?.total ?? '—'} color="purple" />
              <StatCard icon={Package} label="Loại hàng tồn kho" value={resourceOverview?.inventory?.total_items ?? '—'} color="green" />
            </div>
          </div>

          {/* Vehicle requests pending */}
          {resourceOverview?.pending_vehicle_requests > 0 && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-center gap-3">
              <AlertTriangle className="text-yellow-500 shrink-0" size={20} />
              <div>
                <p className="text-sm font-semibold text-yellow-800">
                  {resourceOverview.pending_vehicle_requests} yêu cầu điều xe đang chờ duyệt
                </p>
                <p className="text-xs text-yellow-600">Vào Tài nguyên → Yêu cầu điều xe để xử lý</p>
              </div>
            </div>
          )}

          {/* Weather alerts - still relevant for resource planning */}
          {weatherAlerts.length > 0 && (
            <div>
              <h2 className="text-base font-semibold text-gray-700 mb-3">Cảnh báo thời tiết</h2>
              <div className="space-y-2">
                {weatherAlerts.slice(0, 3).map((a, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-red-50 border border-red-100 rounded-xl text-sm">
                    <CloudLightning className="text-red-400 shrink-0" size={16} />
                    <span className="font-medium text-red-700">{a.province_name}</span>
      ...
      {user?.role !== 'manager' && (
        <>
          ...
          {displayWeatherAlerts.length > 0 && (
            <div className="space-y-2">
              {displayWeatherAlerts.slice(0, 3).map(alert => (
                <div key={alert.id}
          ...
          {/* Weather Widget - chỉ coordinator và rescue_team leader được xem */}
          {weatherConfigured && !(user?.role === 'rescue_team' && !user?.is_team_leader) && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {/* Coordinator & leader: auto-load theo GPS / tỉnh, chỉ hiện tên */}
                  {user?.role === 'coordinator' || (user?.role === 'rescue_team' && user?.is_team_leader) ? (
                    <span className="text-sm text-gray-500 px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-lg flex items-center gap-1">
                      {weatherProvince === '__geo__'
                        ? <><MapPin size={13} className="text-blue-500" /> Vị trí của bạn</>
